...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage): Promise<void> => {
        ...
        const n = [
            './assets/videos/in-love-with/in-love-with.mp4',
            './assets/videos/in-love-with/in-love-with4.mp4',
            './assets/videos/in-love-with/in-love-with3.mp4',
            './assets/videos/in-love-with/in-love-with2.mp4',
            './assets/videos/in-love-with/in-love-with1.mp4'
        ]
        let hug = n[Math.floor(Math.random() * n.length)]
        return void this.client.sendMessage(M.from, { url: hug }, MessageType.video, {
            mimetype: Mimetype.gif,
            caption: `@${user1.split('@')[0]} is in love with @${user2.split('@')[0]}`,
