...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
        super(client, handler, {
            ...
            category: 'fun',
            usage: `${client.config.prefix}hug @whom_you_wanna_kill`
        })
    }
    run = async (M: ISimplifiedMessage): Promise<void> => {
        ...
        const n = [
            './assets/videos/kill/kill.mp4',
            './assets/videos/kill/kill1.mp4',
            './assets/videos/kill/kill2.mp4',
            './assets/videos/kill/kill3.mp4'
        ]
        let hug = n[Math.floor(Math.random() * n.length)]
        return void this.client.sendMessage(M.from, { url: hug }, MessageType.video, {
            mimetype: Mimetype.gif,
            caption: `@${user1.split('@')[0]} killed @${user2.split('@')[0]}`,
